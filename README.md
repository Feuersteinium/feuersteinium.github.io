![GitHub Repo stars](https://img.shields.io/github/stars/einfeuerstein/feuerstein.dev?style=for-the-badge)
![GitHub last commit](https://img.shields.io/github/last-commit/einfeuerstein/feuerstein.dev?style=for-the-badge)
![GitHub Workflow Status](https://img.shields.io/github/actions/workflow/status/feuersteinium/feuerstein.dev/deploy.yml?style=for-the-badge)



# feuerstein.dev
This is the source code of my website, build with [Astro](https://astro.build).

If you found any typos, bugs or mistakes you are free to open a Issue/Pull request. If you don't know how to do this or don't have the time for this, contact me.

## How does the deployment work?

The Website is build via Github Actions, whereby a build is triggered after every push to the main branch.

- Main Site: https://feuerstein.dev 

## License
All **content** on the website, including text, images, and other media, is licensed under the **CC BY-SA License**. All **Source Code** of the website is licensed under the **MIT License**.

Fonts used:
     - Martian Mono (https://fonts.google.com/specimen/Martian+Mono)   
     - Raleway (https://fonts.google.com/specimen/Raleway)  
     - Spacegrotesk (https://fonts.google.com/specimen/Space+Grotesk)  
     - Inter (https://fonts.google.com/specimen/Inter)  

Color Palette:
  - Cyan [#3C6E71]
  - Purple [#291528]
  - Grey [#272626]

